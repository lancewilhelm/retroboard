...
COMMIT=d8778b5f044b216acef2dec723bc9bdcd227cd8d
...
apt-get install -y --force-yes python-pillow python3-dev python3-pillow
...
if [ $QUALITY_MOD -eq 0 ]; then
	...
	make clean
	make install-python HARDWARE_DESC=adafruit-hat-pwm USER_DEFINES="$USER_DEFINES" PYTHON=$(which python)
else
	...
	# Do over for Python 3...
	make clean
	make install-python HARDWARE_DESC=adafruit-hat USER_DEFINES="$USER_DEFINES" PYTHON=$(which python)
fi
